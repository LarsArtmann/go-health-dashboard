...
| Task                                                                         | Status    | Impact | Effort | Notes                                                                                                     |
| ---------------------------------------------------------------------------- | --------- | ------ | ------ | --------------------------------------------------------------------------------------------------------- |
| File the upstream go-health `Check.Since`/`Duration` issue from the draft    | 🔴 `TODO` | Medium  | S      | Draft at `docs/upstream/go-health-check-timestamps-issue-draft.md`; run the verify-before-filing pass first |
...
| CV-side adoption: bump go.mod, deploy, verify live page  | 🔵 `BLOCKED` | Deploy pipeline + rollout order are the user's call (report g Q2)                                                                        | `~/projects/CV` pins older dashboard       |
