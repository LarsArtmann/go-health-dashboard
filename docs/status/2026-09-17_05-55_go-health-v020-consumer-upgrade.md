...
1. **Commit hygiene** — the CODE story is fully committed with intent (3 commits), but the daemon won the
...
4. **Performance measurement** — `buildViewModelAt` adds a per-row stamping loop executed on every pusher
...
1. **Push** — 7 commits sit unpushed; CI has seen **nothing** of this session or yesterday's buildflow
   session. The FEATURES drift guard (256) is only machine-checked on push.
2. **Release decision** — a feature release (metadata UI + new metric) sits complete in `[Unreleased]`.
   Candidate: v0.9.0. Versioning/cut is a user decision; `verify-release.sh` is ready.
3. **vulncheck** — `nix run .#vulncheck` not run after the dependency change (low risk: additive release,
   but it's part of the standard dep-bump checklist and was skipped).
4. **Coverage delta** — the CI Coverage floor will judge on push; no local pre-check was run.
5. **Aggregate-transit integration test** — upstream locks merged `since` fields in its aggregate golden;
   the dashboard-side end-to-end (stub aggregate sources with detailed checks → rendered metadata) is
   untested here.
6. **Fuzz seeds** — new formatters (`formatCheckDuration`, `formatStateAge`) have unit tests but no seeds
   added to the existing fuzz corpus.
7. **Example server demo** — the example doesn't use a detailed-check source, so the live demo shows no
   durations (the best showcase for the feature).
8. **README screenshots** — `docs/screenshot.png` (light + dark) predate the metadata line; the README
   bullet now describes UI the screenshot doesn't show. Regeneration is env-guarded
   (`SCREENSHOT_OUTPUT*`) and not run.
9. **deploy/ asset check** — whether a Grafana dashboard or similar lives in `deploy/` that should gain a
...
4. **Dep-bump verification checklist should be written down** — build+test+race+lint+buildflow existed in
...
6. **Patch-path content assertions** — golden files cover initial HTML; a small test asserting the SSE
...
1. 🔴 **Push master** — 7 unpushed commits, two sessions of work invisible to CI (drift guard, coverage
   floor, version guard all unexercised on this tree).
2. 🔴 **Decide + cut the release** (v0.9.0 candidate: per-check metadata UI + gauge) — `nix fmt` after
   last generate, tag-first-then-master, `bash scripts/verify-release.sh v0.9.0`.
3. 🟠 `nix run .#vulncheck` on the v0.2.0 tree.
4. 🟠 Local coverage pre-check (`nix run .#coverage`) so the CI Coverage floor can't surprise.
5. 🟠 Regenerate README screenshots (light + dark) so the screenshot shows the metadata line.
6. 🟠 Patch-content test: assert the SSE patch payload contains the metadata line (closes b2 for all
   future view changes).
7. 🟠 Benchmark before/after for the per-tick stamping loop (`BenchmarkHandler_HTMLRendering`).
8. 🟡 Example server: add a `NewWithDetailedCheck`-style source so the demo shows durations.
9. 🟡 Integration test: metadata transits the aggregate path end-to-end (stub aggregate + detailed
   sources → rendered row metadata).
10. 🟡 Fuzz seeds for `formatCheckDuration`/`formatStateAge` edge inputs (negative, huge, sub-µs).
11. 🟡 Timeline card fed by service-reported `Since` (draft adoption path item 2 — design first: what
    happens when the probe restarts and Since resets?).
12. 🟡 Stable-group collapse with honest "stable for 6h" summary line (draft item 3; depends on 11's
    design since stability ages come from Since).
13. 🟡 Evidence tooltips cite `Check.Since` (probe truth) alongside the dashboard-observed last non-pass —
    strengthens the health-washing story with service-side facts.
14. 🟡 Consider `since`/`duration` in `/health/export` (JSON/CSV) — design decision: wire-stability
    gotcha says JSON contracts stay go-health-shaped; export is dashboard-owned and could add them.
15. 🟡 Check `deploy/` for Grafana/monitoring assets that should gain the new gauge panel.
16. 🟢 Harvest this report's f-list into TODO_LIST.md / ROADMAP.md (`docs-health` HARVEST) — the two
    scoped-out draft items MUST land there.
17. 🟢 Write the dep-bump checklist down (see e4; possibly fleet-level `verify-dep-bump.sh`).
18. 🟢 AGENTS.md: consider one-line reflex reminder at the top of the release-discipline gotcha: "commit
    beats the daemon — verify batch → commit immediately" (it's already implied; make it unmissable).
19. 🟢 Annotate the 2026-09-16 status reports (14:55 + 18:13) with pointers to this report (continuation
    chain).
20. 🟢 Fleet ask: templ-components + go-datastar relax `go 1.26.7` → `go 1.26` so this module's floor can
    normalize (mirrors go-health v0.2.0's relaxation).
21. 🔵 [BLOCKED-reminder] CV-side adoption (deploy pipeline decision) — unchanged.
22. 🔵 [BLOCKED-reminder] Copy affordance for raw check keys (product decision) — unchanged.
23. 🔵 [BLOCKED-reminder] Pin-guard keep sign-off — unchanged.
24. 🔵 [BLOCKED-reminder] Build-tag gating for SSE (accept/fork/gate) — unchanged.
25. 🔵 [BLOCKED-reminder] Fingerprint format stability versioning decision — unchanged.
26. 🔵 [BLOCKED-reminder] Unskip go-structure-linter (fleet pin bump) — unchanged.
27. 🔵 [BLOCKED-reminder] Unskip branching-flow (fleet decision) — unchanged.
28. 🔵 [BLOCKED-reminder] Unskip templ-generate in BuildFlow (fleet DAG ordering) — unchanged.
29. 🟢 Public-mode README section: note that since-stamps/durations stay visible (non-identifying) —
    aligns docs with the tested behavior.
30. 🟢 `doc.go` package doc: one sentence on per-check metadata (public-API discoverability).
31. 🟢 Webhook payload pin: a golden asserting the webhook JSON passes through v0.2.0 fields unchanged
    (locks the "wire stays go-health-shaped" claim against future drift).
32. 🟢 Public-mode design review: duration VALUES in metrics could theoretically fingerprint an
...
33. 🟢 Mobile visual check of the metadata line in the stacked-card layout (axe passed; human-eye pass on
    a real viewport screenshot would confirm readability).
34. 🟢 Dark-mode contrast spot-check for the new dim metadata line (reuses existing gray-500 tokens, but
    verify WCAG AA like the v0.8.0 banner work).
35. 🟢 CHANGELOG: at tag time, re-head `[Unreleased]` in the SAME change as the Version const bump
    (version-guard job will fail otherwise — standing rule, listed as a guardrail for the release task).
36. 🟢 Consider `WithTrend` interplay: nothing to change (trend samples overall status only) — write the
    one-line rationale where trend is documented to prevent future "why no durations in trend" questions.
37. 🟢 Dashboard-owned export CSV: if 14 lands, mirror in CSV with a `since_age` column decision.
38. 🟢 investigate whether `Dashboard.HealthCheck` (do lifecycle) should surface aggregate check-metadata
    quality (e.g. zero-duration checks when a detailed source was expected) — potential silent-misconfig
    detector; needs design.
39. 🟢 Upstream courtesy (go-health): the v0.2.0 CHANGELOG references go.dev/issue/71631 — when consuming
...
40. 🟢 Consider documenting the metric-name reservation: `dashboard_health_check_duration_seconds` (histogram)
    vs `..._last_duration_seconds` (gauge) in metrics.go doc comment so nobody "unifies" them later.
41. 🟢 Roadmap: "health-washing thesis" follow-up — with probe-truth Since available, re-evaluate whether
...
43. 🟢 Example: `DEMO_*` env toggles pattern — if the example gains a detailed source, validate any new
    env input via `safeBasePath` pattern (standing gotcha).
44. 🟢 jsdelivr/CDN not used — nothing. (Sanity entry: no network deps to review.)
45. 🟢 Consider adding the new gauge to the metrics README example block for the public-mode variant too
    (masked-label example line).
46. 🟢 Re-run `buildflow -s golangci-lint` after the next templ regenerate to ensure generated-file churn
    never reintroduces findings (paranoia gate).
47. 🟢 ROADMAP: when go-health ships aggregated per-source durations for aggregate sources, consider
    per-source worst-duration display (requires upstream work; note only).
48. 🟢 docs/research HTML deep-dive (v0.1.2) is stale — mark superseded by v0.2.0 (point-in-time doc;
    annotate, don't rewrite).
49. 🟢 Keep `verify-release.sh` in sync if the release adds new verification steps (vulncheck at tag time?).
50. 🟢 Fun long-shot: since-age enables "flap detection" (N state changes within M minutes) as a future
    badge state — ROADMAP idea only, no commitment.

## g) Questions I can NOT figure out myself

1. **Release now or accrue?** v0.9.0 candidate (per-check metadata UI + new gauge) is complete in
...
2. **Push the 7 unpushed commits?** CI has seen nothing since yesterday morning; pushing is what makes
...
3. **The two scoped-out draft items** (timeline fed by service-reported `Since`; stable-group collapse
