...
3. **The `retry` field is set per-event but never varies** — `renderPatch` stamps the same `retry` on every event. This is correct for SSE semantics, but the retry value is a pusher-level concern, not a per-patch concern. It could be set once as a standalone `WriteRetry` call when the SSE connection opens, rather than on every event.

4. **`WithRetryInterval` takes `time.Duration` but SSE `retry` is milliseconds** — The API is ergonomic (Duration), but the conversion to `uint` milliseconds happens inside `renderPatch`. If someone passes `500*time.Microsecond`, the `Milliseconds()` call returns 0, silently dropping the retry field. No validation or minimum.

### Testing

5. **SubscriberCount test uses `time.Sleep(100ms)` for disconnect timing** — Same fragility as the reconnection test. Should use `Eventually()` pattern or poll with short intervals.

6. **No negative test for `WithMaxSSEConnections(0)`** — Zero means unlimited, but no test explicitly verifies that `maxConns == 0` actually allows unlimited connections. The existing test only verifies `maxConns == 1` rejects the 2nd client.

7. **No test for SSE handler when `probe.Start()` hasn't been called** — `sseHandler` loads the pusher, but `currentResponse()` reads `probe.CachedResponse()` which returns a zero-value `health.Response` when the probe hasn't started. The dashboard would render an empty/zero dashboard. No test verifies this degraded state.
...
9. **CONTRIBUTING.md wasn't updated** — It doesn't reference `RegisterRoutes`, but it also doesn't mention the new options or the `GOEXPERIMENT=jsonv2` requirement prominently enough (it's buried mid-paragraph).

10. **No godoc example for `WithBasePath`** — The option has good doc comments but no runnable example. Users discovering the API via godoc won't see a usage pattern.

11. **DOMAIN_LANGUAGE.md not updated** — "BasePath", "RetryInterval", "SubscriberCount" are domain terms that should be in the glossary.
...
7. Replace `time.Sleep` in SubscriberCount test with polling/assertion helper
8. Add `TestWithMaxSSEConnections_ZeroAllowsUnlimited` test
9. Add test for SSE handler when probe hasn't started (degraded state rendering)
10. Add `style=` assertion to SSE nonce test (not just `<script`)
11. Add test verifying SSE `retry` field is `uint` and handles large values correctly
12. Add benchmark for `renderPatch` with retry field (perf impact of per-event stamping)
13. Add test for `WithBasePath("/")` and `WithBasePath("")` edge cases
14. Add test for `WithBasePath("/admin/")` (trailing slash) — verify trimming
15. Add test for nested base path `WithBasePath("/a/b")`

### API improvements (from section e)

16. Refactor `WithBasePath` to store a `BasePath` field, resolve in `New()` after all options run
17. Consider `Routes()` method returning configured routes for external mux wiring
18. Consider setting `retry` once per connection instead of per-event
19. Add `WithRetryInterval` doc note about minimum useful value (100ms)
