...
| #  | Item                                                                                                                                                                                                                                                                                                                                                                                                    | Evidence                                                                                                                                                                                                           |
| -- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1  | ~~**Docs-health AUDIT executed per skill** — SKILL.md + references (report format, verify checklist, harvest guide, resolving-items tooling) loaded; all 36 `2026-0*` files examined~~ done — pushed and verified since (v0.6.0+ CI runs green; CHANGELOG Verified sections)                                                                                                                            | skill flow; recent reports read in full, older annotated reports triaged via targeted reads + residual scans                                                                                                       |
| 2  | ~~**Pin defense, round 1** — VERIFY found an undocumented `go get` sweep (templ-components v1.12.0, go-datastar v0.5.0) failing 3 CSP-invariant tests (`TestCSP_NoNonceRendersWithoutNonce`, `TestCSP_EmptyNonceRendersWithoutNonce`, `TestCSP_WithoutDatastarSrcUsesCDN`)~~ done — v0.6.0 shipped 2026-09-04 (latest v0.8.1)                                                                           | root cause verified at source in the module cache: v1.12.0 `liveRegionBusyScript` renders `<script nonce={ nonce }>` unguarded (upstream #7 unfixed); pins restored (`8cf2c62`) + nolint placement fix (`ad2447e`) |
| 3  | ~~**Pin defense, round 2** — a concurrent sweep re-landed the same bumps (`0862981`); re-restored same session (`e607a31`). go-health v0.1.1 → v0.1.2 patch bump (`2761f76`) verified with the full race suite and **kept**~~ done — pages created 2026-09-04 (verified via `gh release list`)                                                                                                          | `go.mod` state at HEAD: templ-components v1.11.0, go-datastar v0.4.0, go-health v0.1.2                                                                                                                             |
| 4  | ~~**CHANGELOG `[Unreleased]` completed** — both pin incidents documented, go-health bump, gate re-verification bullet, bisect-audit path fixed to `archived/`~~ done — `scripts/check-ui-pins.sh` shipped, CI-wired, caught five/six unguarded sweeps                                                                                                                                                   | `40a1778` + follow-up edit in `e607a31`                                                                                                                                                                            |
| 5  | ~~**AGENTS.md re-audited and de-drifted** — Status v0.4.0 → **v0.5.0**; test inventory 19 → **20 files** + `integration_test.go`; templ-components **pin gotcha** (with the twice-bitten evidence); `safeBasePath` env-toggle convention; go-health version note~~ done — CI FEATURES drift guard + `scripts/pre-push-checks.sh` (by v0.8.0)                                                            | `381d64e` + `fabf293`                                                                                                                                                                                              |
| 6  | ~~**FEATURES.md rebuilt** — counts **165/20** (counting command preserved), new **Multi-Service and Integration** section (`Prober`, aggregate, cookbook), webhook rows, sentinel-family row, honest CI row (runner-unverified jobs called out), v0.5.0 Released row (proxy-resolved), Known Gaps: pin + GitHub-Releases gap~~ done — #7 nonce guard fixed upstream, shipped in v1.13.2                 | `381d64e`, `7f4763a`                                                                                                                                                                                               |
| 7  | ~~**README.md completed** — `DEMO_PUBLIC` / `DEMO_BASE_PATH` toggle rows (drift fix from the sweep report), `Updated <time>` observation-time semantics, `HealthCheck` sentinel-family example, dependency version matrix~~ done — #6 fixed upstream, shipped in v1.16.0                                                                                                                                | `381d64e`                                                                                                                                                                                                          |
...
| 10 | ~~**CONTRIBUTING.md** — 75% coverage floor (baseline 76.9%) + `nix run .#coverage` documented~~ done — CI hygiene job runs generate + fmt + diff (2026-09-05 marathon)                                                                                                                                                                                                                                  | this session                                                                                                                                                                                                       |
...
| 13 | ~~**Duplicate test removed** — `TestHealthCheckWithContext_InterfaceSatisfied` deleted per its own 04-42 disposition; `dashboard.go:70-71` compile-time assertions are the strictly stronger check~~ done — devShell provides Chromium (`GO_HEALTH_DASHBOARD_CHROME`); browser suite runs locally                                                                                                       | suite green after removal; count corrected 166 → 165 (`7f4763a`)                                                                                                                                                   |
| 14 | **Reference graph repaired** — 6 stale refs to moved files rewritten, 5 `feedback/new/` → `feedback/archived/`, 1 missing `archived/` segment, 1 self-introduced wrong prefix fixed; full link sweep green                                                                                                                                                                                              | sweep rerun: only illustrative `...md` examples, the _proposed_ `docs/release-checklist.md` filename, and one Go-generic false positive remain                                                                     |
...
1. ~~**CI verification of the 2026-09-04 job additions** — version-guard, coverage~~ done — all verified on real runner runs after the v0.6.0 push (CHANGELOG 0.6.0 Verified); version-guard green on the v0.7.0 tag-first cut
   ...
   ~~rule).~~
2. ~~**Upstream templ-components work** — #6 and #7 have verified diagnoses and~~ done — both fixed upstream: #6 shipped in v1.16.0, #7's nonce guard in v1.13.2
...
4. ~~**Coverage floor** — 75% floor + artifact exist in `ci.yml`; the floor has~~ done — floor evaluated on real runs since v0.6.0 and raised 75→78% on 2026-09-04
   ~~never actually been evaluated by a real runner run, and race-mode coverage~~
   ~~may differ from the 76.9% baseline.~~
5. ~~**Two frozen dead refs** in archived history (`docs/content-negotiation-design.md`,~~ **Won't implement — frozen history, edit value ≈ 0 (this report's own verdict)**
...
6. ~~**gopls stdversion warnings** persist (8 visible in editor diagnostics despite~~ done — dismissed via `analyses.stdversion: false` in `.vscode/settings.json` (2026-09-04); the ROADMAP note records the re-check condition
   ~~the committed `.vscode` env); real lint is clean. Routed to ROADMAP Theme 5.~~
7. ~~**Feature-count stability** — corrected twice in 24h (154→166 by the sweep,~~ done — CI FEATURES drift guard + `scripts/pre-push-checks.sh` enforce the count (in place by the v0.8.0 cycle; the guard caught real drift during the v0.8.0/v0.8.1 cut)
   ~~166→165 by my test deletion). The counting command is documented in FEATURES,~~
   ~~but nothing enforces it.~~

## c) NOT STARTED

1. ~~**v0.6.0 release** — CHANGELOG `[Unreleased]` is release-ready; the cut is a~~ done — v0.6.0 shipped 2026-09-04; latest is v0.8.1
   ~~TODO_LIST row (push → watch new CI jobs → re-head → tag → proxy-verify).~~
2. ~~**GitHub Releases pages** for v0.2.0–v0.5.0 (only v0.1.0 has one).~~ done — pages created 2026-09-04 (verified via `gh release list`, 2026-09-17)
3. ~~**templ-components#6 PR** (StatCard `<dl>` fix + goldens) and **#7 PR**~~ done — both fixed upstream (#6 shipped in v1.16.0, #7's nonce guard in v1.13.2)
   (busy-script nonce guard) — the latter gates any UI-dep re-land.
4. ~~**UI-dep re-land** — the bumps failed twice today; they can re-land only after~~ done — re-landed with browser-suite ceremonies: v1.13.2/v0.5.0 (2026-09-09), v1.16.0 (2026-09-10), v1.17.0 (2026-09-16)
...
7. ~~**A CI pin-guard** asserting the UI-dep versions until #7 lands (new idea from~~ done — `scripts/check-ui-pins.sh` shipped and CI-wired; caught five/six unguarded sweeps since (2026-09-10, 2026-09-16)
   today's double collision — see f6).

## d) TOTALLY FUCKED UP

1. **I shipped a wrong FEATURES count in the same pass that caused it.** I wrote
   "166 functions" into FEATURES and _then_ deleted the redundant test in the
...
2. **Fabricated line numbers in the 04-41 annotation run.** I derived "line
   numbers" for items 34–50 from a _relative_ `sed` range instead of absolute
