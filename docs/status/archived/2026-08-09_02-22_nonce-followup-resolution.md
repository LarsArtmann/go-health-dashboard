...
**Conclusion**: keep `'unsafe-inline'` for the global CSP. A future per-route stricter CSP for `/health` alone is _possible_ (the evidence supports it) but would reintroduce route-specific CSP complexity that the nonce refactor deliberately removed — not worth it. Revisit only if a security audit demands it.
