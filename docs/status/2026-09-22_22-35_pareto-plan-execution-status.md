...
*Brainstorm list, impact-ordered, marked: [NEW] born this session, [ROW] existing TODO_LIST item, [FLEET] lives outside this repo, [DECISION] needs user input.*
...
*Items 1–5 + 7–13 are the true "next session" queue; 6 gates most of them. Items 16+ are the pre-existing parallel backlog — do not re-plan here (docs-health HARVEST routes them).*
